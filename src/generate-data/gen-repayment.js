...
export default async function genRepaymentData(opts) {
  const { data, numData, repsDataPath } = opts;
  let _data = [];
  const dataPath = `./data/${repsDataPath}/data-repayment-sign.json`;
  for (let i = 0; i < data.length; i++) {
    ...
    const apiRoute = "/api/integration/v1.0/debit/repayment-host-to-host";
